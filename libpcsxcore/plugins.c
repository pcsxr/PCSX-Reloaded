...
int LoadPlugins() {
	...
	if (!strcmp("Disabled", Config.Net) || !strcmp("", Config.Net))
		Config.UseNet = 0;
	else {
		Config.UseNet = 1;
		Plugin = g_build_filename (Config.PluginsDir, Config.Net, NULL);
	...
	printf("  CDR: Setting CD file to %s\n", cdrfilename);
	CDR_setfilename(cdrfilename);
