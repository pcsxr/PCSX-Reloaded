...
#ifdef __x86_64__
	//Note: this has to be synced with the locations chosen in ix86_64/iR3000A-64.c
	//Picked a spot somewhat close to a random function psxMemWrite32
	psxM = mmap(&psxMemWrite32 - 0xc000000, 0x00220000,
...
void psxMemReset() {
	...
	if (strcmp(Config.Bios, "HLE")) {
		bios = g_build_filename (Config.BiosDir, Config.Bios, NULL);
		f = fopen(bios, "rb");
