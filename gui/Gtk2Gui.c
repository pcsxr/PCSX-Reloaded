...
enum {
	RUN_CD_BIOS,	/* Run emulation with CD through BIOS */
	RUN_CD,			/* Run emulation with CD */
	RUN				/* Run emulation with no CD in console */
};
...
void OnFile_RunExe();
void on_file_run_cd (GtkWidget *widget, gpointer user_data);
void OnEmu_Reset();
void OnConf_Mcds();
void OnConf_Cpu();
void OnConf_Net();
...
void StartGui() {
	...
	g_signal_connect_data(GTK_OBJECT(widget), "activate",
			GTK_SIGNAL_FUNC(on_file_run_cd), (gpointer) RUN_CD_BIOS, NULL, G_CONNECT_AFTER);

	widget = glade_xml_get_widget(xml, "RunCdBios");
	g_signal_connect_data(GTK_OBJECT(widget), "activate",
			GTK_SIGNAL_FUNC(on_file_run_cd), (gpointer) RUN_CD, NULL, G_CONNECT_AFTER);
	...
	g_signal_connect_data(GTK_OBJECT(widget), "activate",
			GTK_SIGNAL_FUNC(on_file_run_cd), (gpointer) RUN, NULL, G_CONNECT_AFTER);
	...
	/* Network configuration has been removed from PCSX
	widget = glade_xml_get_widget(xml, "netplay1");
	g_signal_connect_data(GTK_OBJECT(widget), "activate",
			GTK_SIGNAL_FUNC(on_configure_plugin), (gpointer) PSE_LT_NET, NULL, G_CONNECT_AFTER);
	*/
	...
	widget = glade_xml_get_widget(xml, "debug1");
	g_signal_connect_data(GTK_OBJECT(widget), "activate",
			GTK_SIGNAL_FUNC(OnConf_Dbg), NULL, NULL, G_CONNECT_AFTER);
	/* Help menu */
...
void ConfigurePlugins() {
	...
	if (strlen(Config.PluginsDir) == 0) {
		if(path = gtk_file_chooser_get_current_folder (GTK_FILE_CHOOSER (widget))) {
	...
	g_signal_connect_data(GTK_OBJECT(widget), "clicked",
			GTK_SIGNAL_FUNC(on_configure_plugin), (gpointer) PSE_LT_PAD, NULL, G_CONNECT_AFTER);
...
void OnFile_RunExe() {
	...
	if (plugins_configured() == FALSE) { 
		...
		gtk_file_filter_set_name (exefilter, "Playstation Executable Files");
		...
		if (gtk_dialog_run (GTK_DIALOG (file_chooser)) == GTK_RESPONSE_ACCEPT) {
			...
			if (OpenPlugins() == -1) {
				...
				if (Load (file) == 0) {
					...
					g_free (file);
					ClosePlugins ();
					SysErrorMessage ("Not a valid PSX file", "The file does not appear to be a valid Playstation executable");
					SysRunGui ();
				}
			}
		} else
...
void on_file_run_cd (GtkWidget *widget, gpointer user_data) {
	...
	if (LoadCdBios != RUN) {
		...
		if (CheckCdrom() == -1) {
			ClosePlugins();
			SysErrorMessage ("CD ROM failed", "The CD does not appear to be a valid Playstation CD");
			SysRunGui();
			return;
		}
	}
	...
	if (LoadCdBios == RUN) LoadCdBios = RUN_CD_BIOS;
...
void OnConf_Dbg() {
	printf("STUB for the debugging interface!\n");
}
...
void OnMcd_Clicked (GtkDialog *dialog, gint arg1, gpointer user_data)
{
	...
	if (arg1 == GTK_RESPONSE_CLOSE)
	{
		...
		if(tmp = gtk_file_chooser_get_filename (GTK_FILE_CHOOSER (widget))) strcpy(Config.Mcd1, tmp);

		widget = glade_xml_get_widget (xml, "GtkMcd2FSButton");
		if(tmp = gtk_file_chooser_get_filename (GTK_FILE_CHOOSER (widget))) strcpy(Config.Mcd2, tmp);
...
int get_free_memcard_slot (int target_card) {
	GtkTreeModel *model;
	McdBlock *Info;
...
//		pixbuf=SetIcon(Info->Icon, i);
		pixbuf=SetIcon(&Info->Icon[*count*16*16], i+1);
...
void OnConf_Mcds() {
	...
	widget = glade_xml_get_widget(xml, "GtkMcd1FSButton");
	gtk_file_chooser_set_filename (GTK_FILE_CHOOSER (widget), Config.Mcd1);
	...
	widget = glade_xml_get_widget(xml, "GtkMcd2FSButton");
	gtk_file_chooser_set_filename (GTK_FILE_CHOOSER (widget), Config.Mcd2);
	g_signal_connect_data(GTK_OBJECT(widget), "selection-changed",
...
void OnCpu_Clicked (GtkDialog *dialog, gint arg1, gpointer user_data)
{
	if (arg1 == GTK_RESPONSE_CLOSE)
	{
		...
		if(Config.Dbg) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(glade_xml_get_widget(xml, "GtkCheckButton_Cpu")), TRUE);
...
void OnConf_Cpu() {
    ...
    gtk_toggle_button_set_state (GTK_TOGGLE_BUTTON (glade_xml_get_widget(xml, "GtkCheckButton_Dbg")), Config.Dbg);
...
/* TODO This should be done when a plugin is first chosen */
#define TestPlugin(src, confs, plugin, name, parent) \
	...
	gchar *filename; \
 \
	dotdir = g_strconcat (getenv("HOME"), PLUGINS_DIR, NULL); \
 \
	GetComboText(confs.Combo, confs.plist, plugin); \
	filename = g_strconcat (dotdir, plugin); \
	drv = SysLoadLibrary(filename); \
	if (drv == NULL) return; \
 \
...
void on_configure_plugin (GtkWidget *widget, gpointer user_data) {
	...
	if (all_config_set() == TRUE) {
		switch (plugin_type) {
			case PSE_LT_GPU:
				OnConfConf_GpuConf (widget, user_data);
				break;
			case PSE_LT_SPU:
				OnConfConf_SpuConf (widget, user_data);
				break;
			case PSE_LT_CDR:
				OnConfConf_CdrConf (widget, user_data);
				break;
			case PSE_LT_PAD:
				OnConfConf_Pad1Conf (widget, user_data);
				break;
			case PSE_LT_NET:
				OnNet_Conf (widget, user_data);
				break;
		}
	} else
		ConfigurePlugins();
...
void on_about_plugin (GtkWidget *widget, gpointer user_data) {
	...
	if (all_config_set() == TRUE) {
		switch (plugin_type) {
			...
			case PSE_LT_PAD:
				PAD1_about();
				if (strcmp(Config.Pad1, Config.Pad2))
					PAD2_about();
				break;
			case PSE_LT_NET:
				/* ADB TODO */
...
void PickDefault (char* confval, char* currchosenval) {;
	/* If no value is set in the config file, then pick the first value in the available plugin list */
	/* TODO Set this when the plugins are first added to the combo list instead */
	if ((currchosenval) && (strlen(confval) == 0)) {
		strcpy(confval, currchosenval);
...
void OnPluginPath_Changed(GtkWidget *wdg, gpointer data) {
	...
	/* ADB TODO Same as set_default_plugin in LnxMain.c!! */
	PickDefault (Config.Gpu, GpuConfS.plist[0]);
	PickDefault (Config.Spu, SpuConfS.plist[0]);
	PickDefault (Config.Cdr, CdrConfS.plist[0]);
	PickDefault (Config.Pad1, Pad1ConfS.plist[0]);
	PickDefault (Config.Pad2, Pad2ConfS.plist[0]);
	PickDefault (Config.Bios, BiosConfS.plist[0]);
	...
	if (arg1 == GTK_RESPONSE_OK)
	{
		...
		*/
		...
		if ((strlen(Config.Gpu) == 0) && (strlen(Config.Spu) == 0) && (strlen(Config.Cdr) == 0) && (strlen(Config.Pad1) == 0) && (strlen(Config.Pad2) == 0) && (strlen(Config.Bios) == 0)) {
			LoadPlugins();		/* TODO Check for return code -1 and handle */
		}
...
void ScanPlugins(gchar* scandir) {
	...
	if (dir == NULL) {
		...
		/* TODO Should throw an error message and a file chooser for a new plugin directory */
	} else {
		printf("Scanning %s for plugins\n", scandir);
		while ((ent = readdir(dir)) != NULL) {
			filename = g_build_filename (scandir, ent->d_name, NULL);

			if ((match(filename, ".*\\.so$") == 0) && (match(filename, "cfg.*") == 0)) {
...
void UpdatePluginsBIOS() {
	...
	dotpluginsdir = g_build_filename (getenv("HOME"), PLUGINS_DIR, NULL);
	check_symlinks_in_path (dotpluginsdir);
	
	/* Check for bad links in ~/.pcsx/plugins/cfg */
	dotpluginscfgdir = g_build_filename (getenv("HOME"), PLUGINS_CFG_DIR, NULL);
	...
	while ((ent = readdir(dir)) != NULL) {
		long type,v;
		linkname = g_build_filename (dotpluginsdir, ent->d_name, NULL);
	...
	//dotdir = g_build_filename (getenv("HOME"), BIOS_DIR, NULL);
	//scan_bios_dir (dotdir);
...
void FindNetPlugin(GladeXML *xml) {
	...
	dotdir = g_strconcat (getenv("HOME"), BIOS_DIR, NULL);
