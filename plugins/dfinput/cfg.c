...
void OnEmulationRadio (GtkWidget *But, gpointer data) {
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (But)))
		pcsx_style = (gint*) data;
	printf("pcsx_style is %d\n", pcsx_style);
}
...
//    ConfWidgets.padnogroup = NULL;
	...
	widget = glade_xml_get_widget (xml, "radiobutton3");
	g_signal_connect_data (GTK_OBJECT (widget), "toggled",
			GTK_SIGNAL_FUNC (OnEmulationRadio), (gpointer) PCSX_STYLE, NULL, G_CONNECT_AFTER);
	widget = glade_xml_get_widget (xml, "radiobutton4");
	g_signal_connect_data (GTK_OBJECT (widget), "toggled",
			GTK_SIGNAL_FUNC (OnEmulationRadio), (gpointer) EPSXE_STYLE, NULL, G_CONNECT_AFTER);
